...
def p_error(p):
    ...
    if p is None:
        f.write("Unexpected EOF :" + str(current_line) + '\n')
    else:
        # if p is not None:
...
def p_error_not_comma_between_tokens(p):
    """rule : NON_TERMINAL ARROW tokens END tokens"""
    f = open(file_out, "a")
    f.write(f"You shouldn't use commas between tokens :" + str(current_line) + '\n')
    f.close()
    correct_line = '`E` = "e"'
    change_line_in_help_file(current_line - 1, correct_line)


def p_error_not_comma_between_tokens_end(p):
    """rule : NON_TERMINAL ARROW tokens END tokens END"""
    f = open(file_out, "a")
    f.write(f"You shouldn't use commas between tokens :" + str(current_line) + '\n')
    f.close()
    correct_line = '`E` = "e",'
